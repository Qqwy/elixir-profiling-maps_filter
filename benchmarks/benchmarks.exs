defmodule Benchmarks do
  ...
  def is_odd_tuple({key, value}) do
    Integer.is_odd(value)
  end

  def is_odd_binary(_key, value) do
    Integer.is_odd(value)
  end

  def run_integer_maps_oddness() do
    Benchee.run(%{
          "Maps.wrapped_filter" => fn input -> MapsFilterProf.wrapped_filter(input, &is_odd_tuple/1) end,
          "Maps.direct_filter" => fn input -> MapsFilterProf.direct_filter(input, &is_odd_tuple/2) end,
          ":maps.filter" => fn input -> :maps.filter(&is_odd_binary/1, input) end,
                },
